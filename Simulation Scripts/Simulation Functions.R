...
design_factors <- data.frame(beta_0 = 20, beta_1 = 1.5, beta_2 = 0.2, beta_3 = 0.01, beta_4 = 1.5,
                             ...
                             trt_ceiling = 1.50, RCT = FALSE, prop_PRIV = 0.05, treatment = "trt",
                             ICC = 0.1, J = 100, testing = FALSE)

simulation_input <- 
  design_factors %>%
  dplyr::mutate(iterations = 2) %>%
  dplyr::mutate(seed = 20230728L + 1:n())

system.time(
  test_results <-
    simulation_input %>%
    dplyr::mutate(res = purrr::pmap(., .f = run_sim)) %>%
